      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          {status === "authenticated" ? (
            <>
              <span className="hidden sm:block text-sm font-medium text-navy">{session.user?.name}</span>
              <button
                onClick={() => signOut({ callbackUrl: "/" })}
                className="text-sm font-semibold px-4 py-2 rounded-lg border border-slate/25 text-navy"
              >
                Log out
              </button>
            </>
            ...
            <>
              <Link href="/login" className="hidden sm:block text-sm font-medium text-navy">Log in</Link>
              <Link href="/signup" className="text-sm font-semibold px-4 py-2 rounded-lg bg-amber text-navy">
                Start free
              </Link>
            </>
          )}
